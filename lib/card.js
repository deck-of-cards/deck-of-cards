...
var upperCaseCardNames = cardNames.map(function ( e ) { return e.toUpperCase(); } );
...
export default function (arg0, arg1) {
  ...
  self.getName = function() { return CardName(self.rank, self.suit); };
  self.getRank = function() { return CardName(self.rank, self.suit).split('of')[0].trim() };
...
function CardNameToIndex(arg0) {
  var words = arg0.split('of').map(function(e) { return e.trim().toUpperCase();});
  if ( words.length === 1 )
      return words[0] === 'JOKER' ? 52 : undefined; //FIXME: All jokers are converted to 52 (the first joker)
